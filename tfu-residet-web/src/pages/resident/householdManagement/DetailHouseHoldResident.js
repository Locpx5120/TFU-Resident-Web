...
const DetailHouseHoldResident = () => {
  ...
  useEffect(() => {
    const fetchAgents = async () => {
      try {
        const response = await fetch(`https://localhost:7082/api/apartment/resident/details/${id}`, {
          method: 'GET',
          headers: {
  ...
  const handleSaveMember = async (memberData) => {
    if (modalMode.mode === 'add') {
      ...
      try {
        const response = await fetch("https://localhost:7082/api/apartment/add-apartment-member", {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${Cookies.get("accessToken")}`,
            'content-type': 'application/json',
